...
class match:
	...
	def allPlayersCompleted(self):
		...
		if self.vinseID is None:
			...
			chat.sendMessage(glob.botName, chanName, "Match history available [{} here]".format(
		...
		if self.isTourney and (chanName in glob.channels.channels):
			chat.sendMessage(glob.botName, chanName, "Match has just finished.")
	...
	def invite(self, fro, to):
		...
		# bot is too busy
		if to == 999:
			chat.sendMessage(glob.botName, froToken.username, "I would love to join your match, but I'm busy keeping ripple up and running. Sorry. Beep Boop.")
	...
	def sendReadyStatus(self):
		...
		chat.sendMessage(glob.botName, chanName, message)
